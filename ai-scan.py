...
async def poker_scan(file: UploadFile = File(...)):
    try:
        ...
        results = pokerModel.predict(source=img, data='data.yaml', conf=0.6, device=DEVICE, save=False, show=False)  # 可调参数
...
async def poker_scan(file: UploadFile = File(...)):
    try:
        ...
        t1 = time.time()
        # YOLO 可以直接传入 PIL Image 或 numpy array
        results = majiangModel.predict(source=img, data='data.yaml', conf=0.6, device=DEVICE, save=False, show=False)  # 可调参数
        print("YOLO耗时:", time.time() - t1)
        # 解析结果
...
async def chip_scan(file: UploadFile = File(...), scan_text: str = Form(...), ):
    try:
        ...
        t1 = time.time()
        # YOLO 可以直接传入 PIL Image 或 numpy array
        results = chipModel.predict(source=img, data='data.yaml', conf=0.6, device=DEVICE, save=False, show=False)  # 可调参数
        t2 = time.time()
        if scan_text == 'yes':
            print("YOLO耗时:", t2 - t1)
        # 解析结果
